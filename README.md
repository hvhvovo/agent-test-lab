...
- **模型出题**：可选 Chat Completions 兼容服务，通过只读检索工具获取资料；限制工具预算，校验参数、输出结构和引用 ID。结合题目中的个人经历前提与否定、计划、团队归属等可疑依据，保守降级为同主题假设场景题；主题不明确时返回澄清题。
...
本地后端验证：**193 项通过，语句覆盖率 98.78%**。检索集包含 **30 条合成标注样例**，分别记录 Recall@K、MRR 和无答案场景结果。指标口径与已知限制见 [测试结果](docs/TEST_RESULTS.md)。
...
批量发现问题：`python -m scripts.batch_deepseek`。使用12个合成开发场景，每个重复3次，共36条任务，顺序执行；两种注入场景使用受控工具结果，其余场景自主检索。只在本机隐藏输入密钥，结果逐条写入reports。Ctrl+C停止后保留已完成记录，重新启动会重新收费执行整批，不支持断点续跑。每轮2000输出Token上限不等于总费用上限。

本批用于发现问题，不能作为独立测试集。先冻结报告，按结构错误、无依据内容、误拦、漏拦归档并人工评分，再集中修复与离线回放，最后另用未见场景验证。重复3次仅观察输出波动，不证明稳定性；受控注入与端到端结果分开报告。


36条真实任务完成34条，生成170道题。对20道重点复核题做离线回归：恢复12道误拦题，处理8道有问题的题（含此前漏拦的7道）。这是已见开发集回归，不是模型总体正确率。`python -m scripts.replay_batch_review 本机原始报告.json` 可核对配套标注，报告哈希不一致会拒绝套用。具体限制见 [经历检查记录](docs/GROUNDING.md)。
